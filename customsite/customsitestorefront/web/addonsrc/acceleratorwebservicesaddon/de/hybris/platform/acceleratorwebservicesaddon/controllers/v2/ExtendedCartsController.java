...
public class ExtendedCartsController
{
	...
	@ResponseBody
	@ApiOperation(value = "Get consolidated pickup options", notes = "Web service handler for getting consolidated pickup options.")
	@ApiBaseSiteIdUserIdAndCartIdParam
	public PointOfServiceListWsDTO getConsolidatedPickupOptions(
			@ApiParam(value = "Response configuration (list of fields, which should be returned in response)", allowableValues = "BASIC, DEFAULT, FULL") @RequestParam(required = false, defaultValue = FieldSetLevelHelper.DEFAULT_LEVEL) final String fields)
	{
	...
	@ResponseBody
	@ApiOperation(value = "Handles the consolidating pickup locations", notes = "Web service handler for consolidating pickup locations.")
	@ApiBaseSiteIdUserIdAndCartIdParam
	public CartModificationListWsDTO consolidatePickupLocations(
			@ApiParam(value = "Name of store where items will be picked", required = true) @RequestParam(required = true) final String storeName,
			@ApiParam(value = "Response configuration (list of fields, which should be returned in response)", allowableValues = "BASIC, DEFAULT, FULL") @RequestParam(required = false, defaultValue = FieldSetLevelHelper.DEFAULT_LEVEL) final String fields)
	...
	@ApiOperation(value = "Get information needed for create subscription", notes = "Method returns information needed for create subscription contacting directly with payment provider. These information "
			+ "contains payment provider url and list of parameters needed to create subscription. Parameters are partially filled with appropriate values.")
	public PaymentRequestWsDTO getSopPaymentRequestDetails(
			@ApiParam(value = "Value for orderPage_cancelResponseURL, 'orderPage_declineResponseURL, orderPage_receiptResponseURL", required = true) @RequestParam(required = true) final String responseUrl,
	...
	@Secured(
	{ "ROLE_CUSTOMERGROUP", "ROLE_GUEST", "ROLE_CUSTOMERMANAGERGROUP", "ROLE_TRUSTED_CLIENT" })
	@ApiOperation(value = "Handles response from payment provider and create payment details", notes = "Method handles response from payment provider and "
			+ "create payment details.\n\nNote, the “Try it out” button is not enabled for this method (always returns an error) because the Extended Carts "
			+ "Controller handles parameters differently, depending on which payment provider is used. For more information about this controller, please "
	...
	public PaymentDetailsWsDTO handleSopPaymentResponse(@ApiIgnore final HttpServletRequest request,
			...
			@ApiParam(value = "Response configuration (list of fields, which should be returned in response)", required = true, allowableValues = "BASIC, DEFAULT, FULL") @RequestParam(required = false, defaultValue = "DEFAULT") final String fields)
	{
	...
	@Secured(
	{ "ROLE_CUSTOMERGROUP", "ROLE_GUEST", "ROLE_CUSTOMERMANAGERGROUP", "ROLE_TRUSTED_CLIENT" })
	@ApiOperation(value = "Get information about create subscription request results", notes = "Method for getting information about create "
			+ "subscription request results. If there is no response from payment provider - method will return 202-Accepted status. If subscription was "
			+ "created successfully payment details is returned. Otherwise error response will be returned.\n\nNote, the “Try it out” button is not enabled "
			+ "for this method (always returns an error) because the Extended Carts Controller handles parameters differently, depending on which payment "
			+ "provider is used. For more information about this controller, please refer to the “acceleratorwebservicesaddon AddOn” documentation on help.hybris.com.")
	@ApiBaseSiteIdUserIdAndCartIdParam
	public PaymentDetailsWsDTO getSopPaymentResponse(@PathVariable final String cartId,
			@ApiParam(value = "Response configuration (list of fields, which should be returned in response)", allowableValues = "BASIC, DEFAULT, FULL") @RequestParam(required = false, defaultValue = "DEFAULT") final String fields,
	...
	@Secured(
	{ "ROLE_CUSTOMERGROUP", "ROLE_GUEST", "ROLE_CUSTOMERMANAGERGROUP", "ROLE_TRUSTED_CLIENT" })
	@ApiOperation(value = "Deletes payment provider response related to cart", notes = "Method deletes payment provider response related to cart.")
	@ApiBaseSiteIdUserIdAndCartIdParam
